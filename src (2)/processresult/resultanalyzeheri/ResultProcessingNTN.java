...
public class ResultProcessingNTN {
...
//        substr = cin.nextLine().split("\\s+");
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ResultProcessingNTN resultProcessingN = new ResultProcessingNTN();
        for (int m = 1; m < 5; m++) {
            // String str1 = "E:\\同步文件\\BaiduSyncdisk\\小论文\\结果集\\启发式算法集\\启发式结果集\\ResultData0" + m + "\\NT_T";
            String str1 = "E:\\同步文件\\BaiduSyncdisk\\小论文\\结果集\\启发式算法集\\改进放置位置后的启发式结果集\\ResultData0" + m + "\\NT_T";
            System.out.println("--------");
            File fileVar1 = new File(str1);
            File[] files = fileVar1.listFiles();
            File fileLB = new File("E:\\同步文件\\BaiduSyncdisk\\小论文\\处理结果集\\数据处理\\NTT.txt");
            // 读取LB
