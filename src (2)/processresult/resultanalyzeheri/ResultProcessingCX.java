...
public class ResultProcessingCX {
...
//        substr = cin.nextLine().split("\\s+");
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ResultProcessingCX resultProcessingCX = new ResultProcessingCX();

        for (int m = 1; m < 5; m++) {
            // String str1 = "E:\\同步文件\\BaiduSyncdisk\\小论文\\结果集\\启发式算法集\\启发式结果集\\ResultData0" + m + "\\CX";
            String str1 = "E:\\同步文件\\BaiduSyncdisk\\小论文\\结果集\\启发式算法集\\改进放置位置后的启发式结果集\\ResultData0" + m + "\\CX";
            System.out.println("--------");
            File fileVar = new File(str1);
            File fileLB = new File("E:\\同步文件\\BaiduSyncdisk\\小论文\\处理结果集\\数据处理\\CX.txt");
            int group = 7;
            // 读取LB
            Map<String, Double> stringDoubleMap = resultProcessingCX.readLB(fileLB);
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (index == 0) {
                    for (int j = 1; j <= files1.length; j++) {
                        ...
                        // 1个一组，计算下界
                        // 读取高度
                        double nowLB = resultProcessingCX.readNowLB(files1[j - 1]);
                        // 下界计算法
                        double nowLB1 = resultProcessingCX.lbo(files1[j - 1].getAbsolutePath());
                ...
                for (int j = 1; j <= files1.length; j++) {
                    // 读取高度
                    double heightTotal = resultProcessingCX.readHeight(files1[j - 1]);
